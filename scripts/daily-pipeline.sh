...
#
# (C5 Review Cycle removed 2026-03-31 — reports were passive with no consumers)
#
# Weekly:
#   Tue & Sat 8:00am SGT: discovery   — discovery-cycle.ts (keyword expansion)
#   Sat  7:30am SGT: flagship-discovery — flagship-discovery.ts (D1 — human-approved via CLI)
...
case "$STEP" in
  ...
  seo)
    ...
    safe_push ;;
  *)
    echo "Usage: $0 {collect|newsletter|extract|flagship-freshness|generate|flagship-discovery|discovery|performance|weekly|cluster-strategy|video-import}"
    echo ""
