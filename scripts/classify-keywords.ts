 ...
 *
 * Uses callClaude (CLI-based) to classify keywords in batches, then cancels off-topic items.
 *
 * Usage:
...
- It's a doc navigation label, not a keyword (e.g., "extensions / get started")
...
async function main() {
  ...
  // ── Step 2: Batch classify with Claude CLI ──
  ...
  for (let i = 0; i < toClassify.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (!jsonMatch) {
        console.warn(`  ⚠ Could not parse response for batch ${batchNum}, defaulting to KEEP`);
      ...
      console.error(`  ⚠ Error on batch ${batchNum}: ${err instanceof Error ? err.message : err}`);
      for (const item of batch) {
        allResults.push({ id: item.job_id, verdict: 'KEEP', reason: 'error' });
      }
    }
  }
