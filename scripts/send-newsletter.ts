...
if (!dateArg) {
  console.error('Usage: npx tsx scripts/send-newsletter.ts --date=YYYY-MM-DD [--lang=en|zh] [--dry-run]');
  process.exit(1);
}
...
console.log(`[send-newsletter] Content length: ${content.length} chars`);

if (dryRun) {
  console.log('[send-newsletter] DRY RUN — not sending.');
  console.log('[send-newsletter] First 200 chars:', content.slice(0, 200));
  process.exit(0);
}
