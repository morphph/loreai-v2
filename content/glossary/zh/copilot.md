...
**Copilot** 泛指嵌入开发环境的 AI 编程助手，能根据上下文自动补全代码、生成函数、解释逻辑。这一概念由 [GitHub Copilot](/zh/glossary/github-copilot) 在 2021 年推出后迅速普及，现已成为 AI 辅助编程工具的代名词——微软、Google、Anthropic 等公司都推出了各自的 copilot 类产品。
...
GitHub Copilot 最初基于 [OpenAI Codex](/zh/blog/codex-complete-guide)，后升级为 GPT-4 系列。Google 的 Gemini Code Assist、Amazon 的 CodeWhisperer 采用类似架构但使用各自模型。核心差异在于上下文窗口大小、支持的语言数量以及与 IDE 的集成深度。

## 相关术语

- **[Claude Code](/glossary/claude-code)**：Anthropic 的终端 AI 编程代理，与 copilot 的行级补全不同，采用 [agentic](/zh/glossary/agentic) 方式执行完整任务
