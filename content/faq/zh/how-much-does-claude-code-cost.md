...
**[Claude Code](/glossary/claude-code)** 本身不单独收费，它的费用取决于你选择的使用方式：通过 [Anthropic](/glossary/anthropic) API 按量付费，或者通过 Claude Pro / Max 订阅计划使用。API 模式下按 token 计费，实际花费取决于你的使用量和选择的模型；订阅模式下则包含在月费中，但有用量上限。
...
Claude Pro（$20/月）和 Claude Max（$100/月或 $200/月）订阅均包含 Claude Code 使用权限。Pro 计划有用量限制，高强度编码会话可能较快触达上限。Max 计划提供显著更高的用量配额，适合将 Claude Code 作为主力开发工具的用户。

订阅模式的优势是费用可预测，不用担心单次大量使用产生高额账单。

## 怎么选

1. **偶尔使用**：API 按量付费最划算，每月可能只花几美元
2. **日常开发**：Claude Pro（$20/月）提供稳定的使用体验，性价比高
