...
4. npm run build 通过
5. Commit + push
6. 在 docs/plans/specs/KEYWORD-ENGINE-LOG.md 记录结果（参考 log 格式见本文档底部）
```
...
**Step 3b — 执行 Spec：**
```
请执行 docs/plans/specs/SPEC-A3-exa.md。（同 Step 2b 的验证要求）
```
...
**Step 4b — 执行 Spec：**
```
请执行 docs/plans/specs/SPEC-A4-gsc.md。（同验证要求）
```
...
**Step 5b — 执行 Spec：**
```
请执行 docs/plans/specs/SPEC-B1-keyword-expansion.md。（同验证要求）
```
...
2. 新 skill: skills/keyword-grouping/SKILL.md（遵循现有 skill 的 SKILL.md 格式）
...
**Step 6b — 执行 Spec：**
```
请执行 docs/plans/specs/SPEC-B2-keyword-grouping.md。（同验证要求）
```
...
**Step 7b — 执行 Spec：**
```
请执行 docs/plans/specs/SPEC-B3-priority-scoring.md。（同验证要求）
```
...
**Step 8b — 执行 Spec：**
```
请执行 docs/plans/specs/SPEC-B4-content-generation.md。（同验证要求）
```
...
**Step 9b — 执行 Spec：**
```
请执行 docs/plans/specs/SPEC-C1-discovery-cycle.md。（同验证要求）
```
...
要求：
1. 修改 write-newsletter.ts，让 blog seed extraction 变为可选
...
**Step 11b — 执行 Spec：**
```
请执行 docs/plans/specs/SPEC-C3-performance-loop.md。（同验证要求）
```
...
2. 修改 scripts/daily-pipeline.sh，新流程：
   ...
   - 5am: write-newsletter.ts（不变）
   - 7am: process-queue.ts（新，替代 write-blog.ts + generate-seo.ts）
   - Saturday: discovery-cycle.ts（新）
   - Tuesday: performance-cycle.ts（新）
3. 保留 write-blog.ts 和 generate-seo.ts 文件但从 cron 中移除
4. npm test 通过
5. Commit + push

参考：docs/plans/KEYWORD-ENGINE-MIGRATION.md Phase C
```
...
- 输出 review HTML（类似现有 generate-review.ts）
