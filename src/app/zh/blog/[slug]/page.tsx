...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const post = getBlogPost(slug, 'zh');
  if (!post) {
    return { title: '文章未找到 | LoreAI' };
  }
  return blogMetadata(
    post.meta.title,
    (post.meta.description as string) || '阅读 LoreAI 博客上的这篇文章。',
    slug,
    'zh'
  );
}
...
export default async function ZhBlogPostPage({ params }: PageProps) {
  ...
  const pageUrl = `https://loreai.dev/zh/blog/${slug}`;
          ...
          <article className="min-w-0">
            ...
            <header className="mb-8">
              ...
              {videoUrl && (
                ...
                >
                  <span aria-hidden="true">&#x1F4FA;</span> 观看视频
                </a>
              )}
            </header>
